...
func TestWorkspaceMigrationBackfillsAllHistory(t *testing.T) {
	...
	before := catalog(t, s)
	if _, err = s.db.Exec(`DROP TABLE workspace_measurements; DROP TABLE workspace_sensors; DROP TABLE workspace_devices; DROP TABLE workspace_layout; PRAGMA user_version=2;`); err != nil {
		t.Fatal(err)
	}
	s.Close()
	s, err = Open(path)
