import { escapeHTML as e, contrast } from "/ui/model.mjs";
...
export function mountBrand(root, notify) {
 ...
 <section class="design-section" id="color">${intro("03 / Color", "Identity, quantity, status", "Chalk and ink carry the interface. Leaf marks actions. Measurement accents identify a quantity, while status labels describe data quality. A copper temperature card does not mean that the temperature is high.")}<div class="swatches">${[
   ["chalk", "Chalk", "#F5F6F2"],
   ["ink", "Ink", "#20342E"],
   ["leaf", "Leaf", "#265D48"],
   ["ochre", "Ochre", "#825513"],
 ...
 <section class="design-section" id="quantities">${intro("03b / Measurement accents", "Recognize before reading.", "Pair a recognizable icon with a stable accent and a readable name. Use the same accent in the card and its history. Unknown quantities keep a neutral device icon; no value is assigned a healthy range automatically.")}<div class="category-grid">${[
   ["temperature", "Temperature", "Copper · thermometer"],
   ["humidity", "Humidity", "Blue · water drop"],
   ["air", "Carbon dioxide", "Violet · air"],
   ["soil", "Soil moisture", "Moss · seedling"],
   ["light", "Illuminance", "Gold · sun"],
   ["device", "Other measurements", "Neutral · device"],
 ]
   .map(
     ([kind, label, description]) =>
       `<div class="category-swatch" data-kind="${kind}">${icon(kind)}<strong>${label}</strong><small>${description}</small></div>`,
   )
   .join(
     "",
 ...
 <section class="design-section">${intro("07 / Voice", "Say what is known.", "Use plain descriptions: “No recent samples”, “Reading error”, “Last known value”. Avoid claiming a device is online just because it reported once. Identify simulations and never mix them into real history.")}<div class="inline-demo"><cj-badge state="ok"></cj-badge><cj-badge state="stale" label="No recent samples"></cj-badge><cj-badge state="error"></cj-badge><cj-badge state="empty"></cj-badge></div></section><footer class="footer"><span>Working visual identity · v0.2</span><a href="/design/research">Research behind the system →</a></footer>`;
  const audit = () => {
...
export function mountComponents(root, notify) {
 ...
 <section class="design-section">${intro("07 / Integration", "Use the same components.", "Load one stylesheet and the component module. No npm installation, compilation or runtime CDN. The application embeds these files; component tests can use Node as a development tool.")} ${code('<link rel="stylesheet" href="/ui/ui.css">\n<script type="module" src="/ui/components.mjs"></script>\n\n<cj-sensor label="Temperature" metric="temperature"\n  value="24.6" unit="degC" state="ok"></cj-sensor>')}<p class="muted small brand-caption">CSS tokens are the public styling surface. Components use light DOM so the application’s styles and accessibility tools can inspect them. Text values are escaped; dynamic scripts and HTML from telemetry are not supported.</p></section>`;
  ...
  const grouped = root.querySelector("#grouped-reading-example");
  for (const model of buildChannels(demoData()).filter(
    (c) => c.sensor === "ambient",
  )) {
    const reading = document.createElement("cj-reading");
    reading.data = { ...model, updated: "1 min ago" };
...
export function mountResearch(root) {
 ...
 ${[
   [
     ...
     "Adopt: readable measurement cards, grouping and progressive detail. Defer: a full visual dashboard editor.",
